...
appRoot.innerHTML = renderSiteMarkup();
...
const OPPOSITE_REBOUND_LOCK_MS = 520;
...
let oppositeScrollLockedUntil = 0;
...
const scrollToTopPosition = (targetTop, behavior = "smooth") => {
  ...
  const step = (now) => {
    ...
    oppositeScrollLockedUntil = lastSnapDirection
      ? performance.now() + OPPOSITE_REBOUND_LOCK_MS
      : 0;
    updateScrollState();
  };
...
const scrollToSnapByDirection = (direction) => {
  ...
  const nextIndex = getNextSnapIndex(direction);

  lastSnapDirection = nextIndex === currentIndex ? 0 : direction > 0 ? 1 : -1;
  scrollToPanelIndex(nextIndex);
};
...
scrollRoot?.addEventListener(
  ...
  (event) => {
    ...
    if (lastSnapDirection && direction === -lastSnapDirection && now < oppositeScrollLockedUntil) {
      resetScrollGestureState();
      return;
    }
...
scrollRoot?.addEventListener("touchend", (event) => {
  ...
  if (lastSnapDirection && direction === -lastSnapDirection && performance.now() < oppositeScrollLockedUntil) {
    resetScrollGestureState({ resetTouch: true });
    return;
  }
