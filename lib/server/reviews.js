...
function normalizeInput(input) {
  ...
  const submittedImageUrls = (parsed.imageUrls.length > 0 ? parsed.imageUrls : [parsed.coverImageUrl]).filter(Boolean);
  const safeImageUrls = submittedImageUrls.map(normalizeImageUrl);

  return {
    ...parsed,
    imageUrls: safeImageUrls,
    coverImageUrl: safeImageUrls[0] || "",
    coverImageAlt: "",
  };
}
